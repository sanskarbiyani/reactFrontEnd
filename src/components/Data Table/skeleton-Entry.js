...
export function NewEntryFinal({ data, fieldData, record_id, fetch, list1 }) {
  ...
  const renderFormElements = (props) =>
    formData[0].map((item, index) => {
      let error =
        props.errors.hasOwnProperty(item.name) && props.errors[item.name];
          ...
          <Grid item sm={12}>
            <Grid container spacing={1}>
              {item.columns.extra_columns.map((loc, i) => {
                if (!props.values[item.name][i]) {
                  props.values[item.name] = Object.assign(
                    props.values[item.name],
                    { [i]: { [loc]: "" } }
                  );
                }
      ...
      if (item.input_type === "hyperlink") {
        if (!props.values[item.name][0]) {
          props.values[item.name][0] = Object.assign({}, { url: "" });
        }
        if (item.columns.text_over_link) {
          if (!props.values[item.name][0]["urlname"]) {
            props.values[item.name][0] = Object.assign(
        ...
        // console.log(props.values[item.name][0]);
        // console.log(props.values[item.name][1]);
        return (
          <Grid item sm={4}>
            <Input
              fullWidth
              ...
              value={
                formValue[0] !== null
                  ? props.values[item.name][0]["url"]
                  : props.values[item.name][1]
              }
              onChange={props.handleChange}
              error={error ? true : false}
              helperText={error}
            ...
            {item.columns.text_over_link && (
              <Input
                ...
                value={
                  formValue[0] !== null
                    ? props.values[item.name][0]["urlname"]
                    : props.values[item.name][1]
                }
                onChange={props.handleChange}
                error={error ? true : false}
                ...
                component={({ field }) => {
                  ...
                  return (
                    <Checkbox
                      {...field}
                      icon={
                        item.columns.is_checkbox ? (
                          <Icon
                            component={MuiIcons["CheckBoxOutlineBlankOutlined"]}
                          ></Icon>
                        ) : (
                          <Icon component={MuiIcons["CircleOutlined"]}></Icon>
                        )
                      }
                      checkedIcon={
                        item.columns.is_checkbox ? (
                          <Icon component={MuiIcons["CheckBoxOutlined"]}></Icon>
                        ) : (
                          <Icon
                            component={MuiIcons["RadioButtonCheckedOutlined"]}
                          ></Icon>
                        )
                      }
                    />
  ...
  formData[0].forEach((item, index) => {
    // console.log(formValue[0][(item.name).replaceAll(/ /g,"_").toLowerCase()])
    if (formValue[0] !== null) {
      initialValues[item.name] =
        item.input_type === "document"
          ? fileDownload(
              `download/${list}/${id}/${item.name.toLowerCase()}/`,
              formValue[0][item.name.replaceAll(/ /g, "_").toLowerCase()]
            )
      ...
      initialValues[item.name] =
        item.input_type === "file"
          ? state
          : item.input_type === "date"
          ? new Date(item.columns.default_date).toJSON().substring(0, 10)
          : item.input_type === "dateTime"
          ? new Date(item.columns.default_date).toJSON()
          : item.columns.default_val || "";
  });
            ...
            <CardContent>
              <Formik
                ...
                onSubmit={(values, actions) => {
                  ...
                  formData[0].forEach((item, index) => {
                    if (item.data_type === "file") {
                      if (values[item.name] instanceof File)
                        formData1.append(
                          item.name.replaceAll(/ /g, "_").toLowerCase(),
                          values[item.name]
                        );
                      else
                        Promise.all([values[item.name]]).then((valuess) => {
                          formData1.append(
                            item.name.replaceAll(/ /g, "_").toLowerCase(),
                            valuess
                          );
                      ...
                      item.input_type === "hyperlink" ||
                      item.input_type === "location" ||
                      item.input_type === "people_group"
                    ) {
                      formData1.append(
                        item.name.replaceAll(/ /g, "_").toLowerCase(),
                        JSON.stringify(values[item.name])
                      );
                    } else
                      formData1.append(
                        item.name.replaceAll(/ /g, "_").toLowerCase(),
                  ...
                  if (formValue[0] !== null) {
                    if (list1 != null) URL += `${list1}/abc/${record_id}/`;
                    else URL += `${list}/xyz/${record_id}/`;
                    axiosInstance

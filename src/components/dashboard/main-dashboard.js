...
function Dashboard()  {
  ...
  const onDelete = (choice) => {
    ...
    axiosInstance.delete(baseURL.concat(listname['list']+"/"+choice+"/"))
    .then(res=>
        {
  ...
  const [listHistory,setListHistory] = useState([JSON.parse(localStorage.getItem('listname'))])
  const [fields, setFields] = useState(initFields);
  useEffect(() => {
    
    axiosInstance.get(baseURL.concat(listname['list']+'/'))
    .then(res=>{
  ...
  return(
  <>
  <Header listname={listHistory} group={listname['group']} select='overview' isList={true}/>
      ...
      <Container maxWidth={false}  >
        ...
        >
          ...
          <Grid
            ...
            xs={12}
          >
            <RenderDataTypeComponent onSubmiting={addFields}  datatypeID= {dataType.type}/>

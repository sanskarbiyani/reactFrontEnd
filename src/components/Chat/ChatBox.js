...
const ChatBox = () => {
  ...
  const [otherUserLoading, setOtherUsersLoading] = useState(false);
  ...
  const addFileMessage = useCallback(
    (fileId, fileName, user, sender = null) => {
      const dateTime = new Date();
      const time = `${dateTime.getDate()}/${dateTime.getMonth()}/${dateTime.getFullYear()} -- ${dateTime.getHours()}:${dateTime.getMinutes()}`;
      const align = user ? "right" : "left";
      const messageObj = {
  ...
  const handleReceivedMsgInput = useCallback(
    (event, open) => {
    ...
    [receiver, dispatch, addFileMessage, addMessage]
  );

  useEffect(() => {
    if (unreadMessages && receiver && receiver.length > 0) {
      dispatch(updateUnreadMessages({ count: 0, userEmail: receiver[0] }));
    }
  }, [receiver, unreadMessages, dispatch]);

  useEffect(() => {
    if (ws) {
      ws.onmessage = (event) => {
        handleReceivedMsgInput(event, true);
      };
    }
    return () => {
      if (ws) {
        ws.onmessage = (event) => {
          handleReceivedMsgInput(event, false);
        };
      }
    };
  }, [ws, receiver, chatOpen, handleReceivedMsgInput]);
  ...
  useEffect(() => {
    axiosInstance
      ...
      .then((response) => {
        ...
        const allUsers = response.map((res) => res.email);
        setAllUsersEmail(allUsers);
      })
      .catch((err) =>
        console.error("Error while fetching emails: ", err.message)
      );
  }, []);
  ...
  const handleSubmit = (event) => {
    ...
    if (fileInput) {
      ...
      axiosInstance
        .post("chat/upload/", formData, config)
        .then((response) => {
          response = response.data;
      ...
      console.log(newUser);
      if (newUser.length === 0) dispatch(addNewOtherUser(receiver));
      ws.send(
        JSON.stringify({
          type: "message",
          message: msg,
  ...
  const get_user_messages = useCallback(() => {
    setOtherUsersLoading(true);
    axiosInstance
      .get(`chat/users/${curr_user.user.email}/`)
      .then((response) => {
        response = response.data;
        let all_users = [];
        let all_messages = {};
        response.forEach((res) => {
          ...
          const ind_messages = messages.map((msg) => {
            ...
            const time = `${dateTime.getDate()}/${dateTime.getMonth()}/${dateTime.getFullYear()} -- ${dateTime.getHours()}:${dateTime.getMinutes()}`;
            const align =
              curr_user.user.email === msg.sender_email.email
                ? "right"
                : "left";
            return {
              align: align,
          ...
          if (user1.email === curr_user.user.email) {
            if (!user2.is_online) user2.is_online = false;
            all_users.push([
              user2.email,
              user2.user_name,
              user2.is_online,
            ...
            if (!user1.is_online) user1.is_online = false;
            all_users.push([
              user1.email,
              user1.user_name,
              user1.is_online,
        ...
        setOtherUserMessages(false);
      })
      .catch((err) => {
        console.error("Error during fetching messages: ", err.message);
        setOtherUserMessages(false);
      });
  }, [curr_user, dispatch]);
  ...
  const getUserNameFromEmail = (event, value, reason) => {
    ...
    if (reason === "selectOption") {
      axiosInstance.get(`chat/getusername/${value}`).then((response) => {
        response = response.data;
        console.log(response);
        if (!response.is_online) response.is_online = false;
        dispatch(
          setReceiver([value, response.user_name, response.is_online, 0])
        );
      });
    }
  };
  ...
  const handleUserChange = (newReceiver) => {
    console.log("Inside User Change Function.");
    ...
    axiosInstance
      .post("chat/messages/", { receiver: newReceiver[0] })
      .then((response) => {
        // console.log(response);
  ...
  useEffect(() => {
    if (!otherUserLoading && otherUsers.length === 0 && curr_user.isLoggedIn) {
      get_user_messages();
      console.log("Got User Messages");
    }
  }, [otherUsers, curr_user, get_user_messages, otherUserLoading]);
            ...
            {otherUsers.map((otherUser, index) => {
              ...
              return (
                ...
                >
                  ...
                  <Typography sx={{ lineHeight: 3, justifySelf: "start" }}>
                    {otherUser[1]}
                  </Typography>

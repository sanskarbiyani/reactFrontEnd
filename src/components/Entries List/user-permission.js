...
function Groups({ UserPermission, RemainingPermission }) {
  ...
  if (
    ...
    !RemainingPermission ||
    RemainingPermission.length === 0
  )
    return <p>Can not find any groups, sorry</p>;
  return (
    <div className="App">
      <Container maxWidth="lg">
        <Paper
          style={{ height: 470, overflow: "auto", border: 1, marginTop: 20 }}
        >
          <Grid
            container
            spacing={2}
...
export function UserPermission({ onChange }) {
  ...
  function PostLoadingComponent({ isLoading, ...props }) {
    ...
    const rem = [];
    if (!isLoading) {
      Object.entries(props["groups"]["index"]["permission"]).map(
        (record, index) => {
  ...
  useEffect(() => {
    axios
      .get(
        "http://127.0.0.1:8000/api/user/permissionOfUserAccGroup/" +
          grp +
          "/" +
          userID
      )

...
const Groups = ({ data, onChange, fetch, searchState, myRules }) => {
  ...
  const onChangeUpdate = (item) => {
    ...
    axiosInstance
      .put(`editModelname/${listdetail["modelname"]}/`, { color: item })
      .then((res) => {
  ...
  const onChangeIcon = (item) => {
    const listdetail = JSON.parse(localStorage.getItem("listname"));
    axiosInstance
      .put(`editModelname/${listdetail["modelname"]}/`, { icon: item })
      .then((res) => {
        console.log(res.data);
        dispatch({
          type: LISTNAME,
  ...
  const colorswap = () => {
    if (colorSwip === "primary") setColorSwip("secondary");
    else setColorSwip("primary");
  };
    ...
    return (
      <>
        <Grid item xs={6}>
          <Card
            elevation={0}
            style={{ borderRight: "0.2px solid lightgrey", borderRadius: 0 }}
          >
            <CardHeader
              title="LISTS"
              action={
            ...
            <Paper style={{ height: 460, overflow: "auto" }}>
              <List>
                <ListItem
                  ...
                  button
                  component={NavLink}
                  to={`/create-new-list/${selectedIndex}/${user["email"]}`}
                >
                  <ListItemIcon>
                    <AddIcon />
                  </ListItemIcon>
                    ...
                    return (
                      <ListItem
                        ...
                        onDoubleClick={() => {
                          ...
                          dispatch({
                            type: GROUP_LIST,
                            group_list: { group: selectedIndex, list: lis[0] },
                          });
                            ...
                            <DialogActions>
                              ...
                              <Button
                                ...
                                onClick={() => {
                                  axiosInstance
                                    .delete(
                                      `deletemodel/ModelSchema/${lis[0]}/`
                                    )
                                    .then((response) => {
                                      // alert(response.data);
                                      // console.log(response.data)
                                      enqueueSnackbar(
                                        `${lis[0]} Delete Successfully`,
                                        {
                                          variant: "error",
                                        }
                                      );
                            ...
                            <FormControl className={classes.formControl}>
                              <Select
                                ...
                                onChange={(e) => {
                                  axiosInstance
                                    .put(
                                      `user/Share-user/${user[2]}/${selectedIndex}/`,
                                      {
                                        group: selectedIndex,
                                        user: user[2],
                              ...
                              >
                                <option
                                  selected={user[1] === "edit" ? true : false}
                                  value={"edit"}
                                >
                                  Can Edit List{" "}
                                </option>
                                <option
                                  selected={user[1] === "view" ? true : false}
                                  value={"view"}
                                >
                                  Can View List
                                </option>
  ...
  const allLists = () => {
    ...
    return (
      <div style={{ width: "100%" }}>
        ...
        <Paper
          style={{ height: 490, overflow: "auto", padding: 30 }}
          elevation={0}
        >
          <Grid container spacing={2}>
            {selectedList.map((lis, index) => {
              return (
                <Grid item xs={3} key={`${lis[0]}_${index}`}>
                  <Card
                    ...
                    <CardActionArea>
                      <CardHeader
                                ...
                                >
                                  ...
                                  >
                                    <Autocomplete
                                      multiple
                                      id="tags-standard"
                                      options={Object.entries(data.groups)
                                        .filter((rec, indexx) =>
                                  ...
                                  onClick={() => {
                                    axiosInstance
                                      .delete(
                                        `links/${lis[0]
                                          .split(" ")
                                          .join("_")}/${selectedIndex}`
                                      )
                                      .then((res) => {
                                        // console.log(res.data)
                                        enqueueSnackbar(
                                          `${lis[0]} Remove Successfully`,
                                          {
                                            variant: "error",
                                          }
                                        );
                        ...
                        subheader={
                          ...
                          >
                            <Grid item xs zeroMinWidth>
                              <Typography noWrap>{lis[3]}</Typography>
                            </Grid>
                          </Grid>
                        }
                      />
        ...
        <Paper style={{ height: 500, overflow: "auto", padding: 30 }}>
          ...
          >
            <Grid item>
              ...
              <Typography>
                Available: {Object.entries(data.personalList).length}
              </Typography>
            </Grid>
            <Grid item>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6} md={4} lg={3}>
              <Card
                className={classes.card}
                elevation={5}
                style={{ maxHeight: 220, minWidth: 300, borderRadius: 15 }}
                onDoubleClick={() => {
                  navigate(`/create-new-list/self/${getUser()}`, {
                    state: { select: "list" },
                  });
                }}
              >
                <CardActionArea>
                  ...
                  />
                  <Divider light />
                  <CardContent className={classes.content} sx={{ height: 67 }}>
                    <>
                      <Grid
                        container
                        justifyContent="space-between"
            ...
            {Object.entries(data.personalList).map((rec, index) => {
              ...
              return (
                ...
                >
                  <Card
                    className={classes.card}
                    style={{ minWidth: 300, maxHeight: 220, borderRadius: 15 }}
                    elevation={5}
                    onDoubleClick={() => {
                      axiosInstance
                        .get("deleteField/".concat(rec[0] + "/"))
                        .then((res) => {
                          const allFields = res.data;
                          console.log(allFields);
                          localStorage.setItem(
                      ...
                      dispatch({
                        type: GROUP_LIST,
                        group_list: { group: "self", list: rec[0] },
                      });
                      navigate(`/list/display-list-data`);
                      dispatch({
                        type: LISTNAME,
                            ...
                            >
                              <div>
                                ...
                                <Dialog
                                  open={
                                    anchorElShare1 &&
                                    Boolean(anchorElShare1[index * 3])
                                  }
                                  onClose={handleClose}
                                >
                                  <DialogTitle
                              ...
                              <div>
                                <MenuItem
                                  key={2}
                                  onClick={() => {
                                    handleClickOpenDeleteList(rec[0]);
                                  }}
                                >
                                  {"Delete List"}
                                </MenuItem>
                                  ...
                                  <DialogActions>
                                    ...
                                    <Button
                                      ...
                                      onClick={() => {
                                        axiosInstance
                                          .delete(
                                            `deletemodel/ModelSchema/${listToBeDeleted}/`
                                          )
                                          .then((res) => {
                                            enqueueSnackbar(
                                              `${listToBeDeleted} Delete Successfully`,
                                              {
                                                variant: "error",
                                              }
                                            );
                                              ...
                                              if (e.target.value.length !== 0) {
                                                axiosInstance
                                                  ...
                                                  .then((res) => {
                                                    ...
                                                    enqueueSnackbar(
                                                      `${e.target.value} Updated Successfully. Please Reload`,
                                                      {
                                                        variant: "success",
                                                      }
                                                    );
        ...
        <Paper style={{ height: 350, width: "100%" }} elevation={0}>
          <Grid container>
            ...
            <Grid item xs={3}>
              ...
              >
                <CardHeader
                  style={{ textAlign: "center", backgroundColor: "#e8e9eb" }}
                  title="MY GROUPS"
                  action={
                    ...
                    {Object.entries(data.groups).map((rec, index) => {
                      return (
                        <ListItem
                          key={index}
                          button
                          selected={selectedIndex === rec[1]["name"]}
                          onClick={() => {
                            setSelectedGroup(rec[1]["name"]);
                            setSelectedList(rec[1]["list"]);
                            setSelectedUser(rec[1]["users"]);
                            handleListItemClick(rec[1]["name"]);
                          }}
                        >
                          <ListItemText
                            id={index}
                            ...
                            style={{
                              color:
                                selectedIndex === index ? "#ff6600" : "#000000",
                            }}
                          />
                          <IconButton>
                            <EditIcon />
            ...
            <Grid item xs={8}>
              {(editButton && <Grid container>{allData()}</Grid>) || allLists()}
            </Grid>
          </Grid>
        </Paper>
      </Container>
      ...
      .then((res) => {
        ...
        axiosInstance
          ...
          .then((res) => {
            ...
            Object.entries(allLists).forEach((entry) => {
  ...
  return (
    <div className="App">
      <PostLoadingComponent
        ...
        personalList={appState.personalList}
        fetch={fetchData}

...
const AccountProfileDetails = ({ props, onSubmiting }) => {
    ...
    const [candidateOptionsFields, setCandidateOptionsFields] = React.useState([]);
    ...
    useEffect(() => {
        ...
        optionsField = [{value: "default", label: "Select Interviewer Field"}, ...optionsField]
        console.log(optionsField);

        let candateFields = allFields.filter(field => {
            if (field.input_type === "string"){
                return field.name;
            }
        })
        candateFields = candateFields.map(field => {
            return {
                "value": field.name,
                "label": field.name
            };
        })
        candateFields = [{value: "default", label: "Select Candidate Name Field"}, ...candateFields]
        console.log(candateFields);
        setNotificationFields(optionsField);
        setCandidateOptionsFields(candateFields);
    }, [])
            ...
            .then((response) => {
                ...
                if (watchNotification){
                    let dataToBeSent = {
                        ...
                        "candidate": data.columns.candidate_name,
                    }
                        ...
                        <Grid container rowSpacing={2} columnSpacing={{ xs: 1, sm: 2, md: 1 }}>
                            <Grid item xs={6}>
                                <Grid container rowSpacing={4} columnSpacing={{ xs: 1, sm: 2, md: 4 }} spacing={2}>
                                    ...
                                    <Grid item xs={11}>
                                        <SelectCustom
                                            {...register('columns.candidate_name')}
                                            id="columns.candidate_name"
                                            variant="outlined"
                                            label="candidate_name"
                                            control={control}
                                            name="columns.candidate_name"
                                            options={candidateOptionsFields}
                                            disable={watchNotification ? false: true}
                                        />
                                    </Grid>
                                </Grid>
